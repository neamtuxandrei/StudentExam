...
namespace ExamSupportToolAPI.Controllers.Student
{
    ...
    public class ExaminationSession : UserBasedController
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(ExaminationSessionForStudent))]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(ErrorResult))]
        [SwaggerOperation(Summary = "Returns current examination session for current student")]
        public async Task<IActionResult> GetExaminationSession()
        {
            try
            {
                ...
                var examinationSession = await _studentService.GetExaminationSession(userExternalId.Value);
